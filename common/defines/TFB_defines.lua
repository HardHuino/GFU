...
NDefines.NGame.MAX_SCRIPTED_LOC_RECURSION = 1001							-- max recursion for scripted localizations, letter mod
...
NDefines.NDiplomacy.BASE_TRUCE_PERIOD = 1 		--Base truce period in days.
NDefines.NDiplomacy.TRUCE_BREAK_COST_PP = 1			--Base cost in PP of breaking a truce by joining a war or accepting a call to war ( you cannot declare war yourself until the truce if up ), this is then multiplied by the time left on the truce ( so once half the truce is up it only cost 50% of this )
NDefines.NDiplomacy.TRUCE_PERIOD_AFTER_KICKING_FROM_FACTION = 1			--	Truce period after kicking someone from faction in days.
...
NDefines.NCountry.NUM_DAYS_TO_FULLY_DELETE_STOCKPILED_EQUIPMENT = 999 		--time in days to fully delete equipments from stockpile. when you delete an equipment, they go to a temporary hidden pool which still can be seized
NDefines.NCountry.BASE_MOBILIZATION_SPEED = 0.12			--	Base speed of manpower mobilization #in 1/1000 of 1 %
...
NDefines.NProduction.ANNEX_FUEL_RATIO = 1	-- How much fuel will be transferred on annexation
NDefines.NProduction.ANNEX_CONVOYS_RATIO = 0.25			-- How many convoys will be transferred on annexation
NDefines.NProduction.CAPITULATE_STOCKPILES_RATIO = 0.6 -- How much equipment from deployed divisions will be transferred on capitulation
...
NDefines.NPolitics.BASE_POLITICAL_POWER_INCREASE = 2.2			--Daily increase of PP.
...
NDefines.NBuildings.INFRA_TO_SUPPLY = 0.5			--	each level of infra gives this many supply
...
NDefines.NMilitary.RECON_SKILL_IMPACT = 6 -- how many skillpoints is a recon advantage worth when picking a tactic.
...
NDefines.NMilitary.TRAINING_ATTRITION = 0.01 --0.02		  			   -- amount of extra attrition from being in training
...
NDefines.NMilitary.PLANNING_MAX = 0.1 --0.15 -- 0.2 in tfb                           	-- can get more from techs
...
NDefines.NMilitary.PROMOTE_LEADER_CP_COST = 0 --25.0					-- cost of promoting a leader
...
NDefines.NMilitary.UNIT_LEADER_ASSIGN_TRAIT_COST = 0 --0.1 --5				-- 	cost to assign a new trait to a unit leader
...
NDefines.NMilitary.UNIT_DIGIN_CAP = 4 --5 in vanilla 		--how "deep" you can dig you can dig in until hitting max bonus
...
NDefines.NAir.FUEL_COST_MULT = 0 --0.17 --0.35
NDefines.NAir.MISSION_EFFICIENCY_MULT_AT_LACK_OF_FUEL = 1		-- 0.25 in vanilla multiplier for mission efficiency when a base lacks fuel
...
NDefines.NAir.ANTI_AIR_ATTACK_TO_DAMAGE_REDUCTION_FACTOR = 0.285			--	Balancing value to convert equipment stat anti_air_attack to the damage reduction modifier apply to incoming air attacks against units with AA.
...
NDefines.NNavy.MISSION_SUPREMACY_RATIOS = { 0.0, 1.0, 0.0, 0.5, 0.5, 0.3, 0.3, 0.0, 0.0, 0.0 } 		-- supremacy multipliers for different mission types HOLD PATROL STRIKE_FORCE CONVOY_RAIDING CONVOY_ESCORT MINES_PLANTING MINES_SWEEPING TRAIN RESERVE_FLEET NAVAL_INVASION_SUPPORT
...
NDefines.NNavy.COMBAT_TORPEDO_CRITICAL_DAMAGE_MULT = 3.0		-- multiplier to damage when got critical hit from torpedo. (Critical hits are devastating as usually torpedo_attack are pretty high base values)
...
NDefines.NNavy.SHORE_BOMBARDMENT_CAP = 0.15 -- shore bombardment limit
...
NDefines.NProject.BASIC_RESEARCH_TECHNOLOGY_BONUS_FACTOR = 0.02     -- Bonus research factor applied to technologies per scientist skill level when performing basic research in a matching facility.
NDefines.NProject.BREAKTHROUGH_DAILY_TECHNOLOGY_GAIN = 12           -- Amount in 1/100th percentage. E.g. 25 = 0.25%
NDefines.NProject.BREAKTHROUGH_DAILY_SCIENTIST_SKILL_GAIN = 5       -- Amount in 1/100th percentage gained per skill when doing basic research. E.g. 5 = 0.05% per skill level.
NDefines.NProject.BREAKTHROUGH_DAILY_ROCKET_SITE_GAIN = 1           -- Amount in 1/100th percentage gained per rocket site level. E.g. 1 = 0.01% per rocket site level.
NDefines.NProject.BREAKTHROUGH_DAILY_NUCLEAR_REACTOR_GAIN = 1       -- Amount in 1/100th percentage gained per nuclear reactor. E.g. 2 = 0.02% per nuclear reactor.
...
NDefines.NProject.RECRUIT_SCIENTIST_COST = {						-- Amount of pp to hire a scientist based on available scientist
	50,			-- pp cost if no available scientist
